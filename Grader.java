...
public class Grader extends Application {
	...
	public void start(Stage primarystage) throws Exception {
		...
		b2.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent arg0) {
				Grader grader = new Grader();
				try {
					grader.loadcourses("src/Courses.csv");
				} catch (FileNotFoundException e) {
					e.printStackTrace();
				}
				System.out.println(grader.getCourses());	
			}
		});
		b3.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent arg0) {
				Grader grader = new Grader();
				try {
					grader.loadevaluations("src/Evaluations.csv");
				} catch (FileNotFoundException e) {
					e.printStackTrace();
				}
				System.out.println(grader.getEvaluation());	
			}
		});
		text1.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent arg0) {
			Student stu = findStudent(text1.getText());
	...
	public Student findStudent(String StudentNumber) {
		for(int i = 0; i< studentlist.size(); i++) {
			if(StudentNumber.equals(studentlist.get(i))) {
